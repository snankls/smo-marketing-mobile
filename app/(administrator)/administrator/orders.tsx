...
export default function AdministratorOrdersScreen() {
  ...
  const fetchOrders = async (
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/orders/administrator?${params.toString()}`, {
        headers: {
  ...
  const renderItem = ({ item }: { item: Order }) => (
    <TouchableOpacity 
      ...
      onPress={() => {
        router.push({
          pathname: "/(administrator)/administrator/order-details",
          params: {
            id: item.id.toString(),
  ...
  return (
    <ScrollView 
      ...
      refreshControl={
        <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[Colors.administrator.primary]} />
      }
    >
      {/* Header */}
      ...
      <View style={styles.filterContainer}>
        ...
        >
          <Ionicons name="refresh-outline" size={18} color={Colors.administrator.primary} />
        </TouchableOpacity>
      </View>
