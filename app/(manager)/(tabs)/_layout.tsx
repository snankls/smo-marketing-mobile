...
function ManagerTabsHeader({ routeName }: { routeName?: string }) {
  const [menuVisible, setMenuVisible] = useState(false);
  const showBack = routeName === "dashboard" ? false : router.canGoBack();
  const showHome = routeName !== "dashboard";
  ...
  const menuOptions = [
    ...
    },
    {
      ...
      onPress: () => {
        setMenuVisible(false);
        router.push("/(manager)/(tabs)/notifications");
      },
      badge: 3,
    },
    {
      ...
      onPress: () => {
        setMenuVisible(false);
        router.push("/(manager)/(tabs)/change-password");
      },
    },
    {
      label: "Logout",
        ...
        <View style={styles.headerLeft}>
          ...
          {showHome && (
            <TouchableOpacity
              ...
              style={styles.headerButton}
            >
              <Ionicons name="home-outline" size={22} color="#C62828" />
            </TouchableOpacity>
          )}
        </View>
...
export default function ManagerTabsLayout() {
  ...
  return (
    <Tabs
      screenOptions={{
        header: ({ route }) => <ManagerTabsHeader routeName={route.name} />,
